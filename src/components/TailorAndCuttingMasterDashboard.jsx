...
const TailorAndCuttingMasterDashboard = () => {
    ...
    const { googleSheetData, fetchPatternAndMrpFromGoogleSheet } = useGlobalContext();
    ...
    //     // Summary Data
    //     const totalQty = uniqueFilteredData.reduce((acc, item) => acc + 1, 0);
    ...
    //     doc.text(`Total Amount: ₹${totalAmount.toFixed(2)}`, 14, 28);
    //     doc.text(`Per Piece Rate: ₹${perPieceRate}`, 14, 34);
    //     doc.text(`Total Days: ${totalDays}`, 14, 40);
    //     doc.text(`Per Day Rate: ₹${perDayRate}`, 14, 46);
    //     doc.text(`Net Payable: ₹${netPayable.toFixed(2)}`, 14, 52);
    ...
    //     // Add a custom font that supports rupee symbol (if needed)
    //     // You might need to add a custom font for better Unicode support
    ...
    //     doc.text(`Net Payable: Rs.${netPayable.toFixed(2)}`, 14, 52);
    ...
    //         theme: "striped",
    //         headStyles: { fillColor: [22, 160, 133] },
    //         styles: { font: "helvetica", fontStyle: "normal" } // Ensure consistent font
    //     });
    ...
    console.log(employeeDressCounts)
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800">
                            Tailor & Cutting Master Dashboard
                        </h1>
                ...
                {activeTab === 'employee' && (
                    <div className="bg-white p-6 rounded-xl shadow-md mb-8">
                        <h2 className="text-xl font-semibold text-gray-800 mb-6">Employee Dress Count by Pattern</h2>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className={`${locationFilter.toLowerCase() === "tailor" ? "block" : "hidden"} px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider`}>Total Amount</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {Object.entries(employeeDressCounts).map(([employee, counts]) => (
                                        <tr key={employee}>
                                            ...
                                            <td className={`${locationFilter.toLowerCase() === "tailor" ? "block" : "hidden"} px-6 py-4 whitespace-nowrap text-sm text-gray-500`}>
                                                ₹{counts.totalRate}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600">
                                            ...
                                            <td className={`px-6 py-4 whitespace-nowrap text-sm text-gray-500`}>
                                                ...
                                                <div className="flex gap-2 ">
                                                    {counts?.workingDays?.map((day) => (
                                                        <span className="bg-red-400 text-white py-1 px-2 rounded-full"> {day.split("-")[2]} </span>
                                                    ))}
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className={` ${locationFilter.toLowerCase() === "tailor" ? "block" : "hidden"} px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider`}>Rate</th>
                                    ...
                                    {uniqueFilteredData.map((item, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td className={`${locationFilter.toLowerCase() === "tailor" ? "block" : "hidden"} px-6 py-4 whitespace-nowrap text-sm text-gray-500`}>
                                                    ₹{matchedPatternAndGetRate(styleDetails?.patternNumber)}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
...
export default TailorAndCuttingMasterDashboard;
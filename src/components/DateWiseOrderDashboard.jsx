...
const DateWiseOrderDashboard = () => {
  ...
  const allDates = [
    ...new Set([...Object.keys(ordersByDate), ...Object.keys(trackingByDate)]),
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {allDates.map((date, index) => {
                  ...
                  const dateOrders = (ordersByDate[date] || []).filter(
                    (order) => selectedPortal === 'All' || order.channel.includes(selectedPortal)
                  );

                  // Filter tracking for the date
                  const dateTracking = trackingByDate[date] || [];
                  const totalOrders = dateOrders.length;
                  const foundInInventory = dateOrders.filter(
                  ...
                  const ship = dateOrders.filter((order) =>
                  ...
                  return (
                    <tr
                      key={date}
                      className={`hover:bg-blue-100 cursor-pointer   ${index % 2 == 0 ? 'bg-blue-50' : ''} `}
                    >

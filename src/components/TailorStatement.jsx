...
const TailorStatement = () => {
  ...
  const fetchStyleWiseData = async () => {
    ...
    try {
      const response = await axios.get(
        'https://stylewise-backend-uqx8.onrender.com/api/v1/stylewise/regular-style/all-styles'
      );
  ...
  const locationTypes = ['Tailor scan 2'];
  ...
  const fetchLocation = async () => {
    ...
    try {
      ...
      allRecords = allRecords.filter(
        (o) => o?.locations?.name?.split(' / ')[0]?.toLowerCase() === 'tailor scan 2'
      );
      let sortedData = [...allRecords].sort(
        (a, b) => new Date(a?.scanned_timestamp - new Date(b?.scanned_timestamp))
      );
  ...
  const filterData = () => {
    ...
    result = result.filter((item) => {
      ...
      return (
        locationName?.toLowerCase()?.includes('tailor scan 2') ||
        locationName.includes('Cutting Master')
      );
    });

    // Second filter: Apply user-selected location filter
    if (locationFilter && locationFilter !== 'All Locations') {
      result = result.filter((item) => item.locations?.name?.includes(locationFilter));
    }
  ...
  const getEmployeeDressCounts = () => {
    ...
    filteredData.forEach((item) => {
      ...
      if (styleNumber) {
        ...
        const rateValue =
          rate && rate.toString().trim().toLowerCase() === 'na' ? 0 : Number(rate) || 0;
  ...
  const exportToPDF = () => {
    ...
    const printStartDate = () => {
      ...
      const sorted = [...uniqueFilteredData].sort(
        (a, b) => new Date(a.scanned_timestamp) - new Date(b.scanned_timestamp)
      );
    ...
    const totalAmount = uniqueFilteredData.reduce((acc, item) => {
      ...
      const pattern = styleDetails?.patternNumber?.toString() || 'NA';
      const rate =
        matchedPatternAndGetRate(pattern) ||
        Number(
          localStorageData.find((item) => Number(item.style_number) === Number(styleNumber))?.rate
        );
      console.log('Matched Rate', rate);
      return acc + (rate === 'NA' ? 0 : rate);
    }, 0);

    const totalDays = Object.values(employeeDressCounts).reduce(
      (acc, val) => acc + val.totalWorkingDays,
      0
    );
  ...
  const addOtherAmount = (order_id, rate, patternNumber, style_number) => {
    ...
    localStorage.setItem(
      'otheramount',
      JSON.stringify([...localStorageData, { order_id, rate, patternNumber, style_number }])
    );
    fetchLocalStorageData();
    setAmount('');
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Tailor Statement Dashboard</h1>
        ...
        {activeTab === 'dashboard' && filteredData.length > 0 && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Employees</h3>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                            <div
                              ...
                              style={{
                                width: `${
                                  (count / Math.max(...Object.values(stats.employeeCounts))) * 100
                                }%`,
                              }}
                            ></div>
                          </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(employeeDressCounts).map(([employee, counts]) => (
                    <tr key={employee}>
                      ...
                      <td
                        className={`${counts.totalRate === 0 ? 'bg-red-200 text-white' : ''} px-6 py-4 whitespace-nowrap text-sm text-gray-500`}
                      >
                        ₹{counts.totalRate}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600">
        ...
        {activeTab === 'details' && (
          <div className="bg-white p-6 rounded-xl shadow-md mb-8">
            ...
            <div className="flex justify-between mb-4">
              <button
                onClick={exportToPDF}
                className="bg-blue-500 text-white py-2 px-4 rounded-md cursor-pointer hover:bg-blue-600 duration-75 ease-in"
              >
                Export Statement
              </button>
              <div className="flex gap-2">
                <button
                  ...
                  className="py-2 px-4 rounded-md cursor-pointer bg-red-500 text-white duration-75 ease-in"
                >
                  Clear Amount
                </button>
                <p className="bg-black text-white text-xl py-2 px-4 rounded-md">
                  {' '}
                  {localStorageData.reduce((acc, cur) => Number(cur.rate) + Number(acc), 0)}{' '}
                </p>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {uniqueFilteredData.map((item, index) => {
                    const styleDetails = matchedDetailsForStyleTypeLiningAndPattern(
                      item.orders_2?.style_number
                    );
                    const manualRate =
                      localStorageData.find((d) => d.order_id === item?.order_id)?.rate || 0;
                    return (
                      <tr
                        key={item?.order_id}
                        className={`${matchedPatternAndGetRate(styleDetails?.patternNumber) === 0 && manualRate === 0 ? 'bg-red-400 text-white' : 'text-gray-500'}`}
                      >
                        <td
                        ...
                        <td
                          className={`${(matchedPatternAndGetRate(styleDetails?.patternNumber) || manualRate) === 0 ? 'bg-red-400 text-white' : ''} px-6 py-4 whitespace-nowrap text-sm text-gray-500`}
                        >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div
                            className={`${(matchedPatternAndGetRate(styleDetails?.patternNumber) || manualRate) > 0 ? 'hidden' : 'flex'}`}
                          >
                            <input
                              ...
                              disabled={matchedPatternAndGetRate(styleDetails?.patternNumber) > 0}
                              className={`${
                                matchedPatternAndGetRate(styleDetails?.patternNumber) > 0
                            ...
                            <button
                              ...
                              className={`${
                                localStorageData.find(
                                  (elem) => Number(elem.order_id) === Number(item.order_id)
                                )
                                  ? 'hidden'
                                  : 'block'
